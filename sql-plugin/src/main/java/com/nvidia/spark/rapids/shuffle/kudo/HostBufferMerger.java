        ...
        if (colOffset.getValidity().isPresent()) {
            ...
            try (HostMemoryBuffer validityBuffer = buffer.slice(offset, validityBufferSize)) {
                ...
                for (int tableIdx = 0; tableIdx < getTableSize(); tableIdx += 1) {
                    ...
                    try (HostMemoryBuffer thisValidityBuffer = validityBufferOf(tableIdx)) {
                        if (thisValidityBuffer != null) {
                            nullCountTotal += copyValidityBuffer(validityBuffer, startRow, thisValidityBuffer, sliceInfo);
                        } else {
        ...
        if (colOffset.getOffset().isPresent()) {
            ...
            for (int tableIdx = 0; tableIdx < getTableSize(); tableIdx += 1) {
                ...
                if (sliceInfo.getRowCount() > 0) {
                    ...
                    IntBuffer thisBuf = offsetBufferOf(tableIdx);

                    int firstOffset = thisBuf.get(0);
                    int lastOffset = thisBuf.get(rowCnt);

                    for (int i = 0; i < rowCnt; i += 1) {
                        buf.put(thisBuf.get(i) - firstOffset + accumulatedDataLen);
                    }
